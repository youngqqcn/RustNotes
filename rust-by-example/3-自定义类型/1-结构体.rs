...
impl Rectangle {
    ...
    fn square(p: Point, x: f32) -> Rectangle {
        Rectangle {
            top_left: Point{ ..p },
            bottom_right: Point{x: p.x + x , y: p.y - x }
        }
    }
...
fn main() {
    ...
    //=============== 作业1
    let rect = Rectangle {
        top_left: Point { x : 0f32, y: 5f32},
        bottom_right: Point { x : 5f32, y: 0f32}
    };
    println!("rect area is : {}", rect.rect_area());

    //=============== 作业2
    let square = Rectangle::square(Point { x : 0f32, y: 5f32}, 10f32  );
